	...
	function fetchWSDL($wsdl) {
    	...
    	while ($imported > 0) {
    		...
    		foreach ($this->schemas as $ns => $list) {
    			foreach ($list as $ilist => $xs) {//<Telinta addition: fixed loop from foreach ($list as $xs)>
					$wsdlparts = parse_url($this->wsdl);	// this is bogusly simple!
		            foreach ($xs->imports as $ns2 => $list2) {
		                for ($ii = 0; $ii < count($list2); $ii++) {
		                	if (! $list2[$ii]['loaded']) {
		                		$this->schemas[$ns][$ilist]->imports[$ns2][$ii]['loaded'] = true;//<Telinta addition: fixed loop from '$this->schemas[$ns]' to '$this->schemas[$ns][$ilist]'>
...
//<Telinta addition>
					} elseif ($parametersArrayType == 'arrayStruct') {
						$this->debug('calling serializeType w/all passed params');
						$xml .= $this->serializeType($name, $type, $parameters, $use, $enc_style);
//</Telinta addition>
					} else {
	...
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			if (is_string($params)) {
				...
				$payload = $params;
			} elseif (is_array($params) or is_object($params)) {
    ...
    function parseResponse($headers, $data) {
		...
		if (strpos($headers['content-type'], '=')) {
			...
			if(preg_match('/^(ISO-8859-1|US-ASCII|UTF-8)$/i',$enc)){
				...
				$this->xml_encoding = 'UTF-8';//<Telinta addition change 'US-ASCII' to 'UTF-8'>
			}
		} else {
			// should be US-ASCII for HTTP 1.0 or ISO-8859-1 for HTTP 1.1
			$this->xml_encoding = 'ISO-8859-1';
