...
class nusoap_client_mime extends nusoap_client {
	...
	*/
	function getHTTPBody($soapmsg) {
		if (count($this->requestAttachments) > 0) {
			...
			$params['content_type'] .= '; start="<main_envelope>"';//<Telinta addition>
			...
			$params['cid']          = 'main_envelope';//<Telinta addition>
			$params['disposition']  = 'inline';//<Telinta addition>
	...
	*/
    function parseResponse($headers, $data) {
		...
		$headers['content-type'] = strtolower($headers['content-type']);//<Telinta addition: strtolower added>
		if (strstr($headers['content-type'], 'multipart/related')) {
			...
			if (!empty($headers['content-transfer-encoding']) and $headers['content-transfer-encoding'] == '8bit') $params['decode_bodies'] = true;//<Telinta addition: added if for 8bit encoded attachments>
			$params['decode_headers'] = true;
			
			$structure = Mail_mimeDecode::decode($params);

			foreach ($structure->parts as $part) {
				if ((!isset($part->disposition) || $part->disposition == 'inline') && (strstr($part->headers['content-type'], 'text/xml'))) {//<Telinta addition: added $part->disposition == 'inline'>
					...
					if (!empty($part->headers['content-id'])) $info['cid'] = $part->headers['content-id'];
