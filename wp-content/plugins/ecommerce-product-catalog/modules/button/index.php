...
class ic_catalog_button {
	...
	function __construct() {
		$this->meta_name	 = '_ic_button_url';
		$this->option_name	 = 'ic_catalog_button';
		$this->menu_element	 = 'affiliate_button';
	...
	function show() {
		...
		if ( !empty( $url ) ) {
			?>
			<div class="ic-affiliate-button-container"><a href="<?php echo $url ?>" class="button"><?php echo $this->settings[ 'label' ] ?></a></div>
			<?php
		}
	}
	...
	function button_url() {
		...
		if ( !empty( $this->settings[ 'individual' ] ) ) {
			$url = get_post_meta( get_the_ID(), $this->meta_name, true );
	...
	function metabox( $post ) {
		if ( !empty( $this->settings[ 'individual' ] ) ) {
			add_meta_box( 'ic_catalog_button', __( 'Button', 'ecommerce-product-catalog' ), array( $this, 'metabox_content' ), 'al_product', 'side', 'default' );
		}
	}
		...
		if ( $submenu == $this->menu_element ) {
			...
			<div class="affiliate-button-settings setting-content submenu">
				<form method="post" action="options.php">
					...
					echo '</table>';
					do_action( 'ic_affiliate_button_settings' );
					?>
					<p class="submit">
	...
	function settings_menu() {
		?>
		<a id="affiliate-button-settings" class="element" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=' . $this->menu_element ) ?>"><?php _e( 'Affiliate Button', 'ecommerce-product-catalog' ); ?> </a><?php
	}
