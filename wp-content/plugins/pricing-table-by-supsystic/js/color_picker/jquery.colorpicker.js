...
(function( factory ) {
	...
	$.widget("vanderlee.colorpicker", {
		options: {
			...
			colorFormat:		'#HEXA',		// Format string for output color format
		...
		getColor: function(format) {
			return this._formatColor(format || '#HEXA', this.color);
		},
		...
		_callback: function (callback) {
			...
			if (that.color.set) {
				data = {
					formatted: that._formatColor('#HEXA', that.color),
					colorPicker: that
				};
		...
		_change: function (stoppedChanging /* = true */) {
			...
			if (!this.inline) {
				if (!this.color.set) {
					...
					this.element.val(this._formatColor('#HEXA', this.color)).change();
				}
