...
class optionsModelPts extends modelPts {
   ...
   public function save($optKey, $val, $ignoreDbUpdate = false) {
      ...
      if (!isset($this->_values[$optKey]) || $this->_values[$optKey]['value'] != $val) {
         if (isset($this->_values[$optKey]) || !isset($this->_values[$optKey]['value'])) $this->_values[$optKey] = array();
   ...
   public function saveGroup($d = array()) {
      ...
      else $this->pushError(__('Empty data to save option', PTS_LANG_CODE));
      return false;
   }
   ...
   private function _loadOptValues() {
      if (!$this->_valuesLoaded) {
         ...
         if (empty($this->_values)) $this->_values = array();
