...
function trusted_dynamic_style( $css = array() ){
	...
	$header_height_home = get_theme_mod( 'header_height_home' );
	if ( $header_height_home && $header_height_home != "580" ) {
		$css[] = '.home .main-header{min-height:'.esc_attr($header_height_home).'px;}';
	}

	$header_height_site = get_theme_mod( 'header_height_site' );
	if ( $header_height_site && $header_height_site != "580" ) {
		$css[] = 'body:not(.home) .main-header{min-height:'.esc_attr($header_height_site).'px;}';
	}
	...
	$tel_no = get_theme_mod( 'tel_no', '' );
					...
					if ( class_exists( 'WooCommerce' ) ) {
						...
						$woo_account_page_id = get_option( 'woocommerce_myaccount_page_id' );
							...
							$cart_contents_total = WC()->cart->get_cart_subtotal();
					...
					<div id="top-info">
						<div class="top-tel">
							...
							<div class="top-cart">
								<a class="cart-contents" href="<?php echo esc_url($woo_top_cart_link); ?>"><i class="fa cart-icon"><?php echo sprintf ( '<span class="item-count">%d</span>', esc_html( WC()->cart->get_cart_contents_count() ) ); ?></i><?php echo $cart_contents_total; ?></a>
...
function trusted_woo_header_add_to_cart_fragments( $fragments ) {
	...
	?>
							...
							<div class="top-cart<?php echo $cart_background;?>">
								<a class="cart-contents" href="<?php echo esc_url($woo_top_cart_link); ?>"><i class="fa cart-icon"><?php echo sprintf ( '<span class="item-count">%d</span>', WC()->cart->get_cart_contents_count() ); ?></i><?php echo $cart_contents_total; ?></a>
								<div class="top-login-mini-cart">
									<?php woocommerce_mini_cart();?>		
								</div>
							</div>
	...
	function trusted_woo_tabs() {
		...
		<section id="home-shop-section" class="section">
			...
			foreach ($tabs as $tab) {
				...
				if ( isset($woo_tabs[$tab_id]['label']) ) {
			...
			foreach ($tabs as $tab) {
				...
				if ( isset($woo_tabs[$tab_id]['shortcode']) ) {
...
if(!function_exists( 'trusted_home_recent_posts' )){
	...
	<section id="recent-posts-section" class="section clearfix">
		<h2 class="recent-posts-title"><?php echo esc_html( get_theme_mod( 'recent_posts_title', __( 'From Our Blog', 'trusted' ) ) ); ?></h2>
